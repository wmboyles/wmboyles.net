...
function nameSchoolAddress(className, addressLineInnerHTML) {
	var schoolAddrs = document.getElementById(className);

	var currDate = new Date();
	var month = currDate.getMonth(); // 4 = May, 7 = August
	var day = currDate.getDate(); // First day = 1

	if((month == 4 && day >= 11) || (month == 5) || (month == 6) || (month == 7 && day <= 13)) { // 5/11-8/13
		schoolAddrs.innerHTML = "N/A in summer";
	} else {
		schoolAddrs.innerHTML = addressLineInnerHTML;
	}
}
